...
export const isCesaAdmin = (auth: AuthInfo | null): boolean => auth?.isCesaAdmin === true;
...
export const hasPermissionAnywhere = (auth: AuthInfo | null, permission: string): boolean => {
  if (!auth) return false;
  if (auth.isCesaAdmin) return true;
  return auth.memberships?.some((m) => m.permissions?.includes(permission)) ?? false;
};
...
export const isDocMember = (auth: AuthInfo | null) => hasRoleInClub(auth, 'ACM', 'Documentation Member');

export const isSecretary = (auth: AuthInfo | null) =>
  hasRoleInClub(auth, 'ACM', 'Secretary') || hasRoleInClub(auth, 'ACM', 'Co-Secretary');
